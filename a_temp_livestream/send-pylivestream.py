...
def open_ffmpeg_stream_process():
    args = (
        "ffmpeg -re -stream_loop -1 -f rawvideo -s 1920x1080 -i pipe:0 -pix_fmt yuv420p -f mpegts udp://127.0.0.1:1234"
    ).split()
...
class Main(App):
    ...
    def doit(self):
        ...
        while (self.do_vid):
            ...
            streamProcess.stdin.write(frame.astype(np.uint8).tobytes())        ### BROKEN PIPE?

            cv2.imshow('Hidden', frame)
            cv2.waitKey(1)
        cam.release()
        streamProcess.stdin.close()
        streamProcess.wait()
...
# https://stackoverflow.com/questions/61615277/is-there-a-way-to-integrate-the-imshow-function-of-opencv-into-kivy-or-kv-file